...
export default function ImcListView() {
  ...
  async function carregar() {
    setLoading(true);
    try {
      const lista = await ImcService.listar();
      setDados(lista || []);
    } catch (err) {
  ...
  async function removerItem(id) {
    try {
      const ok = await ImcService.remover(id);
      if (ok) {
  ...
  function renderItem({ item }) {
    return (
      <Card style={styles.card}>
        <Card.Title
          title={`Peso: ${item.peso} kg, Altura: ${item.altura} m`}
          subtitle={`IMC: ${item.imc.toFixed ? item.imc.toFixed(2) : item.imc}`}
        />
        <Card.Content>
